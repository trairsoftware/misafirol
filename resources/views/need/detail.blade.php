                ...
                <div class="row">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <div>
                                <a class="btn btn-success" href="{{url('/needs')}}">Listeye Geri Dön</a>
                            </div>
                        </div>
                        <div class="card-body">
                            @if (isset($action) && $action === 'edit')
                                <form method="POST" action="{{ url('need/edit/'. $need->id) }}"
                                    ...
                                    @else
                                        <form method="POST" action="{{ url('need/add') }}"
                                            ...
                                            <div class="text-right" style="margin-top: 1rem">
                                                <a class="btn btn-success" href="{{url('need/request/add/' . $need->id )}}">Yardımda Bulun</a>
                                            </div>
                                        </form>
                        </div>
                    </div>
